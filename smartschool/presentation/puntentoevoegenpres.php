...
<!-- al de javascript files die gebruikt worden-->
       ...
       <script src="http://maxcdn.bootstrapcdn.com/bootstrap/3.3.2/js/bootstrap.min.js"></script>
        <title>Smartschool > Punten toevoegen</title>
        <script>
    $('#nav').affix({
});
    </script>
...
<!--  begin form test-->
                 <h2>punten invullen voor <?php print($toets); ?></h2>
<!-- De form die de aanwezigheden opvangt en doorgeeft aan de database-->
<form method="post"  ng-controller="validationApp" class="form-horizontal Verwerken VerwerkenWit" action="puntentoevoegen.php?action=afw&toets=<?php echo $toets ?>" id="inlogform">
                          <a class="btn btn-success btn-sm btnVormgeving" href="toetsenpagina.php?vak=<?php echo $vak ?>"><< terug naar toetsenlijst</a>    
                        <input  class="btn btn-success btn-sm" type="submit" value="punten Toevoegen"/>
...
<div class="row">
        ...
        <div class="thumbnail">
            ...
            <div class="caption">
                ...
                <input class="form-control" type="number" value="<?php if(isset($puntenlijst[$j])){ echo $puntenlijst[$j]->getPunten();} ?>" name="punten<?php  echo $j ?>"/><div class="input-group-addon"> / <?php echo $test->getPuntentotaal(); ?></div>
                </div>
                </div>
        </div>
      </div>
